...
/* ============================================================================
 *  CONVENIOS – UI DE CUOTAS + ACTIVACIÓN Y REGISTRO DE PAGO (SPA INFALIBLE)
 *  (sin SCRIPT_ID; Observer + Hook a cargarGastosComunes + Interval de garantía)
 *  ========================================================================== */
...
async function _gsInvoke(functionName, parameters = []) {
  try {
    if (typeof window.llamarAPI === 'function') {
      return await window.llamarAPI(functionName, parameters); // Execution API
    }
...
function el_(html) { const d=document.createElement('div'); d.innerHTML=html.trim(); return d.firstElementChild; }
...
function findFiltersCard_(host){
  ...
  const by=labels.find(l=>/parcela|parc/i.test(l.textContent||'')); 
  return (by && (by.closest('.card')||by.closest('.form-grid')||by.closest('div'))) || host.querySelector('.card') || host;
}
function findParcelaSeleccionada(){
  const host=getGastosHost_();
  const inputs=Array.from(host.querySelectorAll('input[type="number"],input[type="text"]')).filter(i=>i.offsetParent!==null);
...
function queryByText_(sel,regex,root=document){ return Array.from(root.querySelectorAll(sel)).find(el=>regex.test((el.textContent||'').trim())); }
function isGastosView_(){
  const host=getGastosHost_();
  return !!( queryByText_('h1,h2,.section-title',/Gastos Comunes/i,host)
...
function ensureBindings(){
  const b1=document.getElementById('pq_guardar');
  if(b1 && !b1._bound){ b1.addEventListener('click',onGuardarPagoCuota); b1._bound=true; }
  const b2=document.getElementById('pq_cerrar');
  if(b2 && !b2._bound){ b2.addEventListener('click',()=>{ const m=document.getElementById('modalPagarCuota'); if(m) m.style.display='none';}); b2._bound=true; }
  const a1=document.getElementById('ac_guardar');
  if(a1 && !a1._bound){ a1.addEventListener('click',onActivarConvenio); a1._bound=true; }
  const a2=document.getElementById('ac_cerrar');
  if(a2 && !a2._bound){ a2.addEventListener('click',()=>{ const m=document.getElementById('modalActivarConvenio'); if(m) m.style.display='none';}); a2._bound=true; }

  const r1=document.getElementById('btn-refrescar-convenio');
  if(r1 && !r1._bound){ r1.addEventListener('click',()=>{ const p=findParcelaSeleccionada(); if(!p) return alert('Ingresa N° de parcela en Filtros.'); _hookConvenioOnParcelaChange(p);}); r1._bound=true; }
  const r2=document.getElementById('btn-activar-convenio');
  if(r2 && !r2._bound){ r2.addEventListener('click',()=>{ const p=findParcelaSeleccionada(); if(!p) return alert('Ingresa N° de parcela en Filtros.'); document.getElementById('ac_parcela').value=p; document.getElementById('ac_fecha').valueAsDate=new Date(); document.getElementById('modalActivarConvenio').style.display='block';}); r2._bound=true; }

  const host=getGastosHost_();
  Array.from(host.querySelectorAll('input[type="text"],input[type="number"]')).forEach(inp=>{
    if(!inp._cvBound){ inp.addEventListener('change',()=>{ const p=findParcelaSeleccionada(); if(p) _hookConvenioOnParcelaChange(p);}); inp._cvBound=true; }
  });
}
...
async function onGuardarPagoCuota(){
  try{
    ...
    const fileEl=document.getElementById('pq_file'); let fileObj=null;
    if(fileEl && fileEl.files && fileEl.files[0]){ const f=fileEl.files[0]; fileObj={name:f.name, mime:f.type||'application/octet-stream', base64:await fileToBase64_(f)}; }
    ...
    const payload={ nParcela:_cvState.nParcela, idConvenio:_cvState.idConvenio, nCuota, fechaPago:fecha, monto, metodo, observacion:obs, enviarComprobante:!!enviar, file:fileObj };
    const res=await _gsInvoke('registrarPagoCuota_GS',[payload]);
    await cargarConvenioYCuotas(_cvState.nParcela);
    const modal=document.getElementById('modalPagarCuota'); if(modal) modal.style.display='none';
    alert('Pago registrado correctamente'+(res?.enviadoA?` y comprobante enviado a ${res.enviadoA}`:''));
  }catch(e){ console.error(e); alert('Error al registrar el pago: '+(e.message||e));}
}
async function onActivarConvenio(){
  try{
    ...
    if(!nParcela || !deuda || !cuotas || !valor || !fecha) return alert('Completa todos los campos requeridos.');

    await _gsInvoke('activarConvenio_GS',[nParcela,{deuda, cuotas, valorCuota:valor, fechaInicio:fecha, interes}]);
    const modal=document.getElementById('modalActivarConvenio'); if(modal) modal.style.display='none';
    await _hookConvenioOnParcelaChange(nParcela);
    alert('Convenio activado y cuotas generadas.');
  }catch(e){ console.error(e); alert('Error al activar convenio: '+(e.message||e));}
}
...
async function cargarConvenioYCuotas(nParcela){
  ...
  _cvState.cuotas.forEach(c=>{
    ...
    const btn=(c.estado==='Pagada')?`<button class="btn btn-secondary" disabled>Pagada</button>`:`<button class="btn" onclick="abrirModalPagarCuota(${c.nCuota})">Registrar pago</button>`;
    tr.innerHTML=`
      <td>${c.nCuota}</td>
...
/* 10) Hook público */
window._hookConvenioOnParcelaChange=async function(nParcela){ try{ await cargarConvenioYCuotas(nParcela);}catch(e){console.warn(e);} };

/* 11) Bootstrap INFALIBLE */
(function bootstrapConvenios(){
  function checkAndInject(){
    if(isGastosView_()){
      if(!document.getElementById('convenio-actions-bar')){ injectConveniosUI(); }
      ensureBindings();
      const p=findParcelaSeleccionada(); if(p) _hookConvenioOnParcelaChange(p);
    }
  }
  ...
  function patchRender(fnName){
    ...
    if(typeof orig==='function' && !orig._cvPatched){
      ...
      window[fnName]._cvPatched=true;
    }
  }
  patchRender('cargarGastosComunes');   // aparece en tus errores previos
  patchRender('renderGastosComunes');   // por si la tienes

  // 5) Intervalo de garantía: 1s, liviano y solo actúa en Gastos
  setInterval(()=>{ 
    if(isGastosView_() && !document.getElementById('convenio-actions-bar')) checkAndInject();
  }, 1000);
})();
