...
async function initializeGapiClient() {
    ...
    // La inicialización del token se hace después
}
...
function gisLoaded() {
    ...
    // Una vez que todo está cargado, mostramos el botón de login
...
async function handleTokenResponse(resp) {
    ...
    // Si el token es válido, iniciamos la aplicación
    await startApp();
}
...
function attachViewListeners(viewName) {
    // Usamos un pequeño retraso para asegurar que el DOM se renderice antes de adjuntar listeners
    setTimeout(() => { 
        try {
            ...
            // El listener para gastos comunes se añade directamente en el HTML con onchange
        } catch(e) {
...
// --- LÓGICA DE VISTAS (Restaurada y funcional) ---

async function loadDashboardView() {
    const [residentsData, paymentsData, expensesData, maintenanceData] = await Promise.all([
        readSheetData(SPREADSHEET_ID, 'Residentes!A2:J'), readSheetData(SPREADSHEET_ID, 'Pagos_GC!A:H'),
        readSheetData(SPREADSHEET_ID, 'Egresos!A:G'), readSheetData(SPREADSHEET_ID, 'Mantenciones!A:H')
    ]);
    const now = new Date();
    ...
    const incomeThisMonth = paymentsData.filter(p => p && p[6] && new Date(p[6]).getMonth() === currentMonth && new Date(p[6]).getFullYear() === currentYear).reduce((sum, p) => sum + (parseFloat(p[5]) || 0), 0);
    const expensesThisMonth = expensesData.filter(e => e && e[1] && new Date(e[1]).getMonth() === currentMonth && new Date(e[1]).getFullYear() === currentYear).reduce((sum, e) => sum + (parseFloat(e[5]) || 0), 0);
    const totalIncome = paymentsData.reduce((sum, p) => sum + (parseFloat(p[5]) || 0), 0);
    const totalExpenses = expensesData.reduce((sum, e) => sum + (parseFloat(e[5]) || 0), 0);
    const cashBalance = totalIncome - totalExpenses;
    const pendingMaintenance = maintenanceData.filter(m => m && (m[5] === 'Pendiente' || m[5] === 'Urgente')).length;
    const monthlyTotals = {};
    ...
    paymentsData.forEach(p => { if (!p || !p[6]) return; const d = new Date(p[6]); const monthKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; if (monthlyTotals[monthKey]) { monthlyTotals[monthKey].income += (parseFloat(p[5]) || 0); } });
    expensesData.forEach(e => { if (!e || !e[1]) return; const d = new Date(e[1]); const monthKey = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`; if (monthlyTotals[monthKey]) { monthlyTotals[monthKey].expense += (parseFloat(e[5]) || 0); } });
