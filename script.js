...
// --- NUEVO FLUJO DE AUTENTICACIÓN (ESTABLE) ---
function gapiLoaded() {
...
async function handleTokenResponse(resp) {
    if (resp.error) {
        console.error("Error en el token:", resp);
...
async function loadDashboardView() {
    const [residentsData, paymentsData, expensesData, maintenanceData] = await Promise.all([
        readSheetData('Residentes!A2:I'), readSheetData('Pagos_GC!A2:H'),
        readSheetData('Egresos!A2:G'), readSheetData('Mantenciones!A2:H')
    ]);
    const now = new Date();
...
function handleResidentTableClick(e) {
    if (e.target.matches('.icon-delete')) {
        ...
        if (confirm('¿Está seguro?')) { deleteSheetRow('Residentes', rowIndex); }
    }
...
async function appendSheetData(sheetName, values) {
    return gapi.client.sheets.spreadsheets.values.append({ spreadsheetId: SPREADSHEET_ID, range: `${sheetName}!A1`, valueInputOption: 'USER_ENTERED', resource: { values } });
}

async function updateSheetRow(sheetName, rowIndex, values) {
    const range = `${sheetName}!A${rowIndex}`;
    return gapi.client.sheets.spreadsheets.values.update({ spreadsheetId: SPREADSHEET_ID, range, valueInputOption: 'USER_ENTERED', resource: { values } });
}
